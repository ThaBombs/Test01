...
TRAILING_MACRO_COMMA_RE = re.compile(
    r'^(?P<body>\s*[A-Za-z_.$][A-Za-z0-9_.$]*\s+[^,]+),\s*SET_SYMBOL_RE = re.compile(
...
def expand_file(path: Path, repo_root: Path, stack: tuple[Path, ...]) -> list[str]:
    ...
    for raw in text.splitlines():
        ...
        # A pair of modern animation scripts use an uppercase "Call". GNU as
        # accepts the project macro spelling more loosely than LLVM; normalize
        # it without changing the script target.
        uppercase_call = UPPERCASE_CALL_RE.match(line)
        if uppercase_call:
            line = f"{uppercase_call.group('indent')}call{uppercase_call.group('rest')}"

        # ANIM_BATTLER appears in Tera Starstorm sprite placement. The
        # createsprite macro only distinguishes target from non-target here,
        # and the corresponding sprite callback anchors to the attacker.
...
UPPERCASE_CALL_RE = re.compile(r'^(?P<indent>\s*)Call(?P<rest>\s+.*)SET_SYMBOL_RE = re.compile(
    r'^(?P<indent>\s*)\.set\s+(?P<symbol>[A-Za-z_.$][A-Za-z0-9_.$]*)\s*,'
)
ENUM_EQUIV_RE = re.compile(
    r'^(?P<indent>\s*)\.global\s+(?P<symbol>[A-Za-z_.$][A-Za-z0-9_.$]*)\s*;'
